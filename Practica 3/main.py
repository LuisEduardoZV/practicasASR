...
from TrendUpdate import update
from trendGraphDetection import monitoreo
...
print("Practica 3 - Monitoreo de Umbrales")
...
while True:
    ...
    if op == 1:
        ...
        index = busca_dispositivo(data, 1)
        dispositivo = data[index]
        oids = ["1.3.6.1.2.1.25.3.3.1.2.196608", "1.3.6.1.2.1.25.2.3.1.6.1", "1.3.6.1.2.1.25.2.3.1.6.1"]
        hiloUpdate = threading.Thread(name='update', 
                         target=update,
                         args=(oids,),
                         daemon=True)
        hiloUpdate.start()
        inventario = ["1.3.6.1.2.1.1.1.0", "1.3.6.1.2.1.1.5.0", "1.3.6.1.2.1.1.4.0", "1.3.6.1.2.1.1.6.0", "1.3.6.1.2.1.2.1.0"]
        table = ["1.3.6.1.2.1.2.2.1.2.", "1.3.6.1.2.1.2.2.1.7."]
        res, res_table = obten_data(inventario, table, dispositivo)
        tipo = input("\nQue desea monitorear?\n1) Carga CPU\n2) Carga RAM\n3) Trafico de red\nOpcion: ")
        
        monitoreo(int(tipo), res)
    elif op == 0:
        print("Saliendo...")
        break
