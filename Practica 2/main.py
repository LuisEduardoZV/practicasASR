...
from graphRRD import createGraph
...
def obten_data_contabilidad(oids, dispositivo):
    res = []
    res_table = []
    index = 1
    version = 0
    ...
    for oid in oids:
        data = consulta_snmp(dispositivo["Comunidad"], dispositivo["IP"], version, puerto, oid).split()
        if index == 1 or index == 3 or index == 4 or index == 5 or index == 6 or index == 7:
            res.append(data[2])
        elif index == 2:
            res.append(" ".join(data[2:]))
        index = index+1
...
def creaPDFContabilidad(res, dispositivo):
    w, h = A4
    now = datetime.now()
    date_time = now.strftime("%m-%d-%Y_%H:%M:%S")
    titulo = "ReporteContabilidad_" + dispositivo['Nombre'] + date_time + ".pdf"
    doc = canvas.Canvas(titulo, pagesize=A4)
    titulos = doc.beginText(30, h-50)
    titulos.setFont("Times-Roman", 18)
    titulos.textLine("Administración de Servicios en Red")
    titulos.textLine("Practica 2")
    titulos.textLine("Luis Eduardo Zúñiga Vera                                                 4CM13")
    doc.drawText(titulos)
    doc.line(10, h-120, w-10, h-120)
    data = doc.beginText(30, h-140)
    data.setFont("Times-Roman", 12)
    data.textLine("Version 1")
    data.textLine(f"Device: {dispositivo['Nombre']}")
    data.textLine(f"Description: {res[1]}")
    data.textLine(f"Date: {date_time}")
    data.textLine("DefaultProtocol: radius")
    data.textLine(" ")
    date_time = now.strftime("%m-%d-%Y_%H:%M:%S")
    data.textLine(f"rdate: {date_time}")
    data.textLine("#NAS-IP-Address")
    data.textLine(f"4: {dispositivo['IP']}")
    data.textLine("#NAS-Port")
    data.textLine(f"5: {dispositivo['Puerto']}")
    data.textLine("#NAS-Port-Type")
    data.textLine(f"61: {dispositivo['Version']}")
    data.textLine("#User-Name")
    data.textLine(f"1: {res[0]}")
    data.textLine("#Acct-Status-Type")
    data.textLine("40: 2")
    data.textLine("#Acct-Delay-Time")
    data.textLine("41: 0")
    data.textLine("#Acct-Input-Octets")
    data.textLine(f"42: {res[3]}")
    data.textLine("#Acct-Output-Octets")
    data.textLine(f"43: {res[4]}")
    data.textLine("#Acct-Session-Id")
    data.textLine("44: 1")
    data.textLine("#Acct-Authentic")
    data.textLine("45: 1")
    data.textLine("#Acct-Session-Time")
    data.textLine(f"46: {res[2]}")
    data.textLine("#Acct-Input-Packets")
    data.textLine(f"47: {res[5]}")
    data.textLine("#Acct-Output-Packets")
    data.textLine(f"48: {res[6]}")
    data.textLine("#Acct-Terminate-Cause")
    data.textLine("49: 11")
    data.textLine("#Acct-Multi-Session-Id")
    data.textLine("50: 73")
    data.textLine("#Acct-Link-Count")
    data.textLine("51: 2")
    doc.drawText(data)
    createGraph()
    doc.drawImage('/home/trophy/PycharmProjects/practicasASR/Practica 2/unicast.png', (w/2)-50, (h/2)+160, 300, 130)
    doc.drawImage('/home/trophy/PycharmProjects/practicasASR/Practica 2/paquetes.png', (w/2)-50, (h/2)+25, 300, 130)
    doc.drawImage('/home/trophy/PycharmProjects/practicasASR/Practica 2/icmp.png', (w/2)-50, (h/2)-110, 300, 130)
    doc.drawImage('/home/trophy/PycharmProjects/practicasASR/Practica 2/segmentos.png', (w/2)-50, (h/2)-245, 300, 130)
    doc.drawImage('/home/trophy/PycharmProjects/practicasASR/Practica 2/datagramas.png', (w/2)-50, (h/2)-380, 300, 130)
    doc.showPage()
    doc.save()
    return 1
...
if os.path.isfile('/home/trophy/PycharmProjects/practicasASR/Practica 2/practicaRRD.rrd') == 0:
    createRRD()
oids_graficas = ["1.3.6.1.2.1.2.2.1.11.2", "1.3.6.1.2.1.4.3.0", "1.3.6.1.2.1.5.8.0", "1.3.6.1.2.1.6.10.0", "1.3.6.1.2.1.7.1.0"]
hilo = threading.Thread(name='updateRRD', 
                         target=updateRRD,
                         args=(oids_graficas,),
                         daemon=True)
hilo.start()
while True:
    ...
    if op == 1:
        ...
        data = abrir_archivo(filename)
        oids = ["1.3.6.1.2.1.1.4.0", "1.3.6.1.2.1.1.6.0", "1.3.6.1.2.1.1.3.0", "1.3.6.1.2.1.2.2.1.10.2", "1.3.6.1.2.1.2.2.1.16.2", "1.3.6.1.2.1.2.2.1.11.2", "1.3.6.1.2.1.2.2.1.17.2"]
        index = busca_dispositivo(data, 1)
        dispositivo = data[index]
        res = obten_data_contabilidad(oids, dispositivo)
        creaPDFContabilidad(res, dispositivo)
        print("\nPDF generado, salga del programa para poder visualizarlo...\n")
    elif op == 0:
        print("Saliendo...")
        break
